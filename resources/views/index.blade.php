<x-layouts.base>
    <main class="pt-16">
        ...
        <section class="py-24 bg-surface" id="staff-tecnico">
            <div class="max-w-7xl mx-auto px-6">
                <div class="mb-12 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        <p class="mb-3 text-sm font-black uppercase tracking-[0.3em] text-secondary">Staff técnico</p>
                        <h1 class="text-5xl font-black font-lexend tracking-tighter text-primary uppercase leading-none">
                            Conocé a los profes
                        </h1>
                        <div class="h-2 w-24 bg-secondary mt-2 rounded-full"></div>
                    ...
                    <p class="max-w-2xl text-lg leading-relaxed text-on-surface-variant">
                        Cada entrenamiento está acompañado por referentes que combinan planificación, experiencia y
                        cercanía para cuidar el proceso deportivo y humano de cada jugador.
                    </p>
                </div>
                ...
                @php
                    $coaches = [
                        [
                            ...
                            'categories' => 'Edefi, Bafi y planificación integral',
                            'experience' => '12 años formando jugadores infantiles y juveniles.',
                            'quote' => 'Primero buenas personas; después mejores futbolistas.',
                            'photo' => 'https://images.unsplash.com/photo-1543351611-58f69d7c1781?auto=format&fit=crop&w=900&q=80',
                        ],
                        [
                            'name' => 'Laura Fernández',
                            'role' => 'Entrenadora de desarrollo',
                            'categories' => 'Edefi y Futsal Femenino',
                            'experience' => 'Especialista en iniciación deportiva, técnica individual y hábitos de equipo.',
                            'quote' => 'La confianza también se entrena todos los días.',
                            'photo' => 'https://images.unsplash.com/photo-1526232761682-d26e03ac148e?auto=format&fit=crop&w=900&q=80',
                        ],
                        [
                            'name' => 'Diego Suárez',
                            'role' => 'Preparador físico',
                            'categories' => 'Bafi, Futsala y alto rendimiento',
                            'experience' => '10 años acompañando procesos físicos seguros para etapas formativas.',
                            'quote' => 'Entrenar bien es aprender a moverse mejor.',
                            'photo' => 'https://images.unsplash.com/photo-1517466787929-bc90951d0974?auto=format&fit=crop&w=900&q=80',
                        ],
                    ];
                @endphp
                ...
                <div class="grid grid-cols-1 gap-8 md:grid-cols-2 xl:grid-cols-3">
                    @foreach ($coaches as $coach)
                        <article class="group overflow-hidden rounded-3xl border border-primary/10 bg-surface-container-lowest shadow-lg transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl">
                            <div class="relative h-80 overflow-hidden">
                                <img alt="{{ $coach['name'] }} - {{ $coach['role'] }}"
                                    class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105"
                                    src="{{ $coach['photo'] }}" />
                                <div class="absolute inset-0 bg-gradient-to-t from-primary/95 via-primary/20 to-transparent"></div>
                                <div class="absolute bottom-0 left-0 right-0 p-6">
                                    <p class="mb-2 inline-flex rounded-full bg-secondary px-3 py-1 text-xs font-black uppercase tracking-widest text-on-secondary">
                                        {{ $coach['role'] }}
                                    </p>
                                    <h2 class="text-3xl font-black leading-tight text-on-primary">{{ $coach['name'] }}</h2>
                                </div>
                            </div>
                            <div class="space-y-5 p-6">
                                <div class="flex items-start gap-3">
                                    <span class="material-symbols-outlined mt-0.5 text-primary">groups</span>
                                    <div>
                                        <p class="text-xs font-black uppercase tracking-widest text-on-surface-variant">Categorías</p>
                                        <p class="font-bold text-on-surface">{{ $coach['categories'] }}</p>
                                    </div>
                                </div>
                                <div class="flex items-start gap-3">
                                    <span class="material-symbols-outlined mt-0.5 text-primary">workspace_premium</span>
                                    <div>
                                        <p class="text-xs font-black uppercase tracking-widest text-on-surface-variant">Experiencia</p>
                                        <p class="text-on-surface-variant">{{ $coach['experience'] }}</p>
                                    </div>
                                </div>
                                <blockquote class="rounded-2xl bg-primary/5 p-5 text-lg font-black italic leading-snug text-primary">
                                    “{{ $coach['quote'] }}”
                                </blockquote>
                            </div>
                        </article>
