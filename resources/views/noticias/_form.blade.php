...
<div class="space-y-4">
    <div><label>Categoría</label><select name="category" class="w-full border rounded p-2">@foreach($categories as $value => $label)<option value="{{ $value }}" @selected(old('category', $publication->category ?? 'institucional') === $value)>{{ $label }}</option>@endforeach</select></div>
