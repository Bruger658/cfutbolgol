...
<div class="space-y-4">
    ...
    <div>
        ...
        <select name="category" class="w-full border rounded p-2 bg-surface text-on-surface" style="background-color:#0b1730;color:#ffffff;border-color:#94a3b8;">
            @foreach($categories as $value => $label)
                <option value="{{ $value }}" class="bg-surface text-on-surface" style="background-color:#0b1730;color:#ffffff;" @selected(old('category', $publication->category ?? 'institucional') === $value)>{{ $label }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="flex items-center gap-3">
        ...
        <a href="{{ route('noticias') }}" class="text-sm text-on-surface-variant hover:text-on-surface">Cancelar</a>
    </div>
</div>
